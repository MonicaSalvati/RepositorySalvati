...
def web_query_to_documents(
    settings: Settings,
    max_results: int = 5
) -> List[Document]:
    """
    Chiede la domanda all'utente, genera keyword (opzionale via LLM),
    cerca su DuckDuckGo (DDGS) e converte i risultati in Document (titolo+snippet).
    NON scarica le pagine: niente problemi SSL.
    """
    question = input("Cosa vuoi cercare? ").strip()
    if not question:
        print("Domanda vuota.")
        return []
    ...
    # 2) ricerca DDGS + 3) trasformazione in Document
    docs: List[Document] = []
    docs = []
...
def main():
    ...
    web_docs = web_query_to_documents(settings, max_results=5)
    ...
    # 5) Esempi di domande
    questions = [
        "Mi fai una sintesi dei documenti?",
